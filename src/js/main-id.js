...
function display() {
  ...
  progressBar(`Pertarungan No. ${battleNo}`, percent);
...
function pick(sortType) {
  ...
  if (leftIndex < 0) {
    ...
    progressBar(`Pertarungan No. ${battleNo} - Selesai!`, 100);
...
function result(imageNum = 10) {
  ...
  const timeStr = `Sorter ini selesai pada ${new Date(timestamp + timeTaken).toString()} dan memakan waktu selama ${msToReadableTime(timeTaken)}. <a href="${location.protocol}//${sorterURL}">Lakukan sorter lainnya?</a>`;
...
function decodeQuery(queryString = window.location.search.slice(1)) {
  ...
  try {
    ...
    console.error(`Galat memuat tautan yang dapat dibagikan: ${err}`);
...
function preloadImages() {
  ...
  const loadImage = async (src) => {
    ...
    return new Promise((res, rej) => {
      ...
      reader.onload = ev => {
        progressBar(`Memuat Gambar ${++imagesLoaded}`, Math.floor(imagesLoaded * 100 / totalLength));
...
function msToReadableTime (milliseconds) {
  ...
  t = t - (minutes * 60);
  const content = [];
	if (years) content.push(years + " tahun" + (years > 1 ? "" : ""));
	if (months) content.push(months + " bulan" + (months > 1 ? "" : ""));
	if (days) content.push(days + " hari" + (days > 1 ? "" : ""));
	if (hours) content.push(hours + " jam"  + (hours > 1 ? "" : ""));
	if (minutes) content.push(minutes + " menit" + (minutes > 1 ? "" : ""));
	if (t) content.push(t + " detik" + (t > 1 ? "" : ""));
